...
@router.get("/users")
def get_all_users(db: Session = Depends(get_db), current_user: dict = Depends(require_manager_or_above)):
...
def update_user_role(user_id: int, body: UpdateRoleRequest, db: Session = Depends(get_db), current_user: dict = Depends(require_admin)):
    return auth_controller.update_user_role(db, user_id, body.role)


@router.put("/users/{user_id}/password")
def update_password(user_id: int, body: UpdatePasswordRequest, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
