...
def create_vendor(db: Session, vendor_name: str, email: str, phone: str,
    ...
    vendor_id = vendor_queries.create_vendor(db, vendor_name, email, phone, hq_address, hq_city, hq_state, hq_zip)
    return {
        "VendorId": vendor_id, "VendorName": vendor_name, "Email": email, "Phone": phone,
...
def update_vendor(db: Session, vendor_id: int, vendor_name: str, email: str, phone: str,
    ...
    if not vendor:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Vendor not found")
    vendor_queries.update_vendor(db, vendor_id, vendor_name, email, phone, hq_address, hq_city, hq_state, hq_zip)
    return {
        "VendorId": vendor_id, "VendorName": vendor_name, "Email": email, "Phone": phone,
        "HQAddress": hq_address, "HQCity": hq_city, "HQState": hq_state, "HQZip": hq_zip
    }
